...
const TopRated = () => {
    ...
    return (
        <section id='pizza' className={`${darkMode ? 'dark bg-black' : 'light bg-white'} h-fit w-full lg:px-20 px-3 lg:py-20 py-10 flex flex-col justify-center items-center gap-6`}>
            ...
            <p data-aos="zoom-in" className='text-xl text-black dark:text-slate-100 text-center'>Stop by and find out what all the buzz is about. Order your favorites today<br></br>or visit our restaurant to create your ideal pizza! </p>

            <div data-aos="slide-up" className='lg:w-[90%] w-full'>
                <Slider {...settings}>
                    {
                        pizza.map((item, index) => (
                            <div key={index} id='product-box' className='bg-black dark:bg-gray-800 p-8 flex-col justify-center items-center gap-4 rounded-lg'>
                                <img src={item.image} alt="" className='rounded-lg transform hover:scale-105 transition-transform duration-300 cursor-pointer' />
